...
class ClientUtil(
  ...
  def submitCommandReturnUnit[T](
      sender: P.Party,
      workflowId: WorkflowId,
      command: P.Update[P.Unit]): Future[Empty] = {
    commandClient.submitSingleCommand(submitRequest(sender, workflowId, command.asInstanceOf[P.Update[P.ContractId[T]]]))
  }
